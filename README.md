...
$$\text{s.t.} \quad \mathbb{E} \left[ \sum_{t=1}^{N} \left( \mathbf{x}_t^\top \mathbf{Q} \mathbf{x}_t - \mathbb{E}[\mathbf{x}_t^\top \mathbf{Q} \mathbf{x}_t | \mathcal{F}_{t-1}] \right)^2 \right] \leq \epsilon$$
...
- **Fourth moment**: \(m_4 = \mathbb{E}\left[\left(\boldsymbol{\delta}_t^\top \mathbf{Q} \boldsymbol{\delta}_t - \text{tr}(\mathbf{Q}\mathbf{W})\right)^2\right]\)
